...
class ViT(nn.Module):
    def __init__(self, d_patch, n_patches, d_model, n_heads, n_blocks, dff, dropout):
        super().__init__()

        self.inproj = nn.Linear(d_patch, d_model)
        self.pe = nn.Parameter(torch.randn(n_patches, d_model))
